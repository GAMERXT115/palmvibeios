...
class _VideoPlayerScreenState extends State<VideoPlayerScreen> with TickerProviderStateMixin, WidgetsBindingObserver {
  ...
  Future<void> _initializePlayer() async {
    try {
      ...
      if (mounted) {
        ...
        _videoPlayerController!.play();
        _videoPlayerController!.setVolume(_volumeBoost.clamp(0.0, 1.0));
        _startHideTimer();
      ...
      if (lines.length >= 3) {
        ...
        if (timingParts.length == 2) {
          ...
          _subtitles.add(SubtitleEntry(
            start: _parseT(timingParts[0].trim()),
            end: _parseT(timingParts[1].trim()),
            text: text,
          ));
        }
      }
  ...
  void _updateSubtitles() {
    ...
    final newText = currentEntry?.text ?? '';
          ...
          content: Column(
            ...
            children: [
              _buildAccRow('Volume Boost', '${_volumeBoost.toStringAsFixed(1)}x', () {
                setState(() { _volumeBoost = math.max(1.0, _volumeBoost - 1.0); _videoPlayerController?.setVolume(_volumeBoost.clamp(0.0, 1.0)); });
                setDialogState(() {});
              }, () {
                setState(() { _volumeBoost = math.min(20.0, _volumeBoost + 1.0); _videoPlayerController?.setVolume(_volumeBoost.clamp(0.0, 1.0)); });
                setDialogState(() {});
              }),
